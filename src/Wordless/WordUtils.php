...
class WordUtils
{
     ...
     {
          return in_array( strtolower( $word ), self::WORDLIST );
     }

     public static function RandomWord() : string
     {
          return self::TodaysWord( -1 );
     }

     public static function TodaysWord( int $dayIndex = 0 ) : string
     {      
          ...
          {  
               ...
               $epoch = new \DateTimeImmutable( '2023-1-1' );
               $diff = $epoch->diff( $now );
               mt_srand( $diff->days );

               $r = mt_rand( 0, count( self::WORDLIST ) - 1 );
          }
          else
          {
               $r = mt_rand( 0, count( self::WORDLIST ) - 1 );
     ...
     private static function MatchesGreen( string $testWord, string $guessChar, int $charIndex ) : bool
     {
     ...
     public static function CountMatches( array $candidates, string $answer, array $guesses )
     {
          ...
          foreach( $candidates as $candidate )
          {
               ...
               {
                    if( !self::IsWordCompatibleWithClues( $candidate, $answer, $guess ) )
                    {
                         $finalIsMatch = false;
                         break;
                    }
